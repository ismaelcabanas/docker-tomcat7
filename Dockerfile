FROM ismaelcabanas/docker-java8
...
# instalamos dependencias para Tomcat Native Libraries
...
# descaga Tomcat y lo descomprime en /opt/tomcat 
RUN set -x \
	\
	&& cd /tmp \
...
# indica a Docker en qué puerto escuchará el proceso
EXPOSE 8080
